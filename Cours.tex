...
\section{Transformée de Laplace du produit de convolution}

Soient $f$ et $g$ deux fonctions causales.

\begin{align*}
	\L{f(t)}\times\L{g(t)} & = \int_0^\infty e^{-pu}f(u)\diff{u}\,\int_0^\infty e^{-pv}g(v)\diff{v}\\
		               & = \int_0^\infty \int_0^\infty e^{-p(u+v)} f(u)\,g(v)\diff{u}\diff{v}\\
			       & = \int_0^\infty \int_0^\infty e^{-pt} f(t-\tau)\,g(t)\diff{\tau}\diff{t}\\
			       & = \int_0^\infty e^{-pt} \underbrace{\int_0^\infty f(t-\tau)\,g(t)\diff{\tau}}_{(f*g)(t)}\diff{t}
\end{align*}

Ainsi, $\L{f*g} = \L{f}\times\L{g}$.

\chapter{Transformée de Laplace inverse}

\section{Définition}

Soit $F(p)$ la transformée de Laplace d'une fonction $f(t)$. On appelle transformée de Laplace inverse, ou original, de $F(p)$, la fonction $f(t)$.

On note $f(t) = \Li{F(p)}$, ou encore $F(p)\xrightarrow{\mathcal{L}^{-1}}f(t)$.

L'existence et l'unicité de l'original ne sont vraies que sous certaines conditions, que l'on admettra vraies pour toute fonction ici.

\begin{itemize}
	\item{La fonction $f$ doit être continue par morceaux pour tout intervalle fermé de $\R^+$.}
	\item{La fonction $f$ doit être d'ordre exponentiel à l'infini.}
\end{itemize}
...
Tout comme la transformée de Laplace, la transformée inverse est linéaire.
\[\forall\ \lambda,\mu\in\R, \Li{\lambda F + \mu G} = \lambda\Li{F} + \mu\Li{G}\]

\subsection{Multiplcation par un réel}

Soit $f(t)$ l'original de $F(p)$. On pose $ax=t$.

\begin{align*}
	F(ap) & = \int_0^\infty e^{-apx}f(x)\diff{x}\\
	      & = \frac{1}{a}\int_0^\infty e^{-pt}f\!\left(\frac{t}{a}\right)\diff{t}
\end{align*}

On en déduit
\[\Li{F(ap)} = \frac{1}{a}f\!\left(\frac{t}{a}\right)\]

\subsection{Avance -- Retard}

On pose toujours $f(t)$ comme l'original de $F(p)$.
\begin{align*}
	F(p+a) & = \int_{0}^\infty e^{-(p+a)t}f(t)\diff{t}\\
	       & = \int_{0}^{\infty} e^{-pt}\big(e^{-at}f(t)\big)\diff{t}
\end{align*}

On en conclut que
\[\Li{F(p+a)} = e^{-at}f(t)\]

\subsection{Dérivation -- Primitivation}

On va ici exprimer $\frac{\partial F(p)}{\partial p}$ en fonction de $f(t)$.

\begin{align*}
	\frac{\partial F(p)}{\partial p} & = \int_0^\infty\frac{\partial}{\partial p}\big(e^{-pt}f(t)\big)\diff{t}\\
		                         & = \int_0^\infty -t\,e^{-pt}f(t)\diff{t}\\
					 & = \int_0^\infty e^{-pt}\big(-t\,f(t)\big)\diff{t}
\end{align*}

On a par conséquent $\Li{\frac{\partial F(p)}{\partial p}} = -t\,f(t)$.

Faisons de même avec la primitivation.

\begin{align*}
	\int_p^\infty F(u)\diff{u} & = \int_p^\infty\left(\int_0^\infty e^{-ut}f(t)\diff{t}\right)\diff{u}\\
	                           & = \int_0^\infty\left(\int_p^\infty e^{-ut}f(t)\diff{u}\right)\diff{t}\\
				   & = \int_0^\infty f(t)\left[\frac{e^{-ut}}{-t}\right]_p^\infty\diff{t}\\
				   & = \int_0^\infty e^{-pt}\frac{f(t)}{t}\diff{t}
\end{align*}

Ainsi, on a
\[\Li{\int_p^\infty F(u)\diff{u}} = \frac{f(t)}{t}\]

\begin{remark}
	\[\Li{F^{(n)}(p)} = (-1)^n\,t^n f(t)\]
\end{remark}

\subsection{Original d'un produit}

Soient $F(p) = \L{f(t)}$ et $G(p) = \L{g(t)}$.
\[\Li{F(p)\,G(p)} = (f*g)(t)\]
où $f*g$ désigne le produit de convolution de $f$ par $g$.

\begin{example}{Original de $\frac{1}{p^2(p+1)}$}

On sait d'après une table de transformée que $\Li{\frac{1}{p^2}}=t$ et que $\Li{\frac{1}{p+1}}=e^{-t}$. On peut alors écrire

\begin{align*}
	\Li{\frac{1}{p^2(p+1)}} & = \int_0^t x\,e^{-(t-x)}\diff{x}\\
	                        & = e^{-t}\int_0^t x\,e^x\diff{x}\\
				& = (t-1)+e^{-t}\\
\end{align*}
\end{example}

\chapter{Applications de la transformée de Laplace}

\section{Applications mathématiques}

Soit l'équation différentielle suivante :
\[a_n y^{(n)}(x) + a_{n-1}y^{(n-1)}(x) + \dotsb + a_0y(x) = f(x)\]

La transformée de Laplace et l'exploitation de ses propriétés nous permettent d'écrire que
\begin{align*}
\L{y(x)}       & = Y(p)\\
\L{\dot{y}(x)} & = p\,Y(p) - y(0)\\
\L{y^{(n)}(x)} & = p^n\,Y(p) - p^{n-1}y(0) - p^{n-2}\dot{y}(0) - \dotsb - y^{(n-1)}(0)
\end{align*}

En appliquant la transformée de Laplace aux deux membres de l'équation, on obtient, par linéarité, l'équation dans le domaine de Laplace suivante :
\[(a_n p^n + a_{n-1} p^{n-1} + \dotsb + a_0)Y(p) + \Phi(p) = F(p)\]
où $\Phi(p)$ désigne un polynôme de degré $n-1$. Après avoir isolé $Y(p)$, on obtient
\[Y(p) = \frac{F(p)-\Phi(p)}{\sum_{k=0}^{n}a_k p^{k}}\]

\begin{example}{Résoudre $\ddot{y} + 2\dot{y} + y = x\,e^{x}$, avec $y(0)=1$ et $\dot{y}(0)=0$.}

	On commence par appliquer à transformée de Laplace au membre gauche de l'équation.
	\begin{align*}
		\L{\ddot{y}-2\dot{y} + y} & = \L{\ddot{y}} + 2\L{\dot{y}} + \L{y}\\
		                          & = (p^2Y(p) - p\,x - 0) - 2(p\,Y(p)-1) + Y(p)\\
					  & = (p^2+2p+1)Y(p) - p+2\\
	\end{align*}

	On transforme ensuite le membre de droite.
	\begin{align*}
		\L{x\,e^x} & = -\left(\frac{1}{p-1}\right)^2\\
		           & = \frac{1}{(p-1)^2}
	\end{align*}

	On peut donc réécrire complètement l'équation dans le domaine de Laplace.
	\begin{align*}
		(p^2+2p+1)Y(p) - p+2 & = \frac{1}{(p-1)^2}\\
		\iff Y(p)            & = \frac{1}{(p-1)^2}\left(\frac{1}{(p-1)^2}+p-2\right)\\
		                     & = \frac{1}{(p-1)^4} - \frac{1}{(p-1)^2} + \frac{1}{p-1}
	\end{align*}

	Enfin, on applique la transformée inverse pour retrouver la fonction dans le domaine temporel.
	\begin{align*}
		y(t) & = e^t \frac{1}{3!}t^3 + e^t - t\,e^t\\
		     & = e^t\left(\frac{t^3}{6}-x+1\right)
	\end{align*}
\end{example}

\begin{remark}
	Cette méthode se généralise à tous les cas d'équations différentielles linéaires à coefficients constants et aux systèmes différentiels linéaires.
\end{remark}

\section{Applications physiques}

On considère un système linéaire invariant.

\begin{figure}[h!]
\begin{center}
	\begin{tikzpicture}
		\draw (0,0) -- node[above]{$x(t)$} (1,0);
		\draw (1,-0.75) rectangle node{Système} (4,0.75);
		\draw (4,0) -- node[above]{$y(t)$} (5,0);
	\end{tikzpicture}
\end{center}
\caption{Système linéaire invariant}
\end{figure}

Dans les systèmes étudiés, $y(t)$ est lié à $x(t)$ par une équation différentielle linéaire à coefficients constants de la forme
\[\sum_{k=0}^{n}a_k\frac{\partial^k\,y(t)}{\partial\,t^n} = \sum_{k=0}^{n}b_k\frac{\partial^k\,x(t)}{\partial\,t^n}\]

Quelque soit le signal d'entrée $x(t)$, le signal de sortie sera de la forme $y(p)=x(t)\times h(t)$, soit, dans le domaine de Laplace, $Y(p) = X(p)\times H(p)$.

$H(p)$ est appelée \emph{fonction de transfert} du système, et c'est elle qui le caractérise. On l'exprime sous la forme $H(p)=\frac{X(p)}{Y(p)}$.

\begin{example}{Application à un circuit $RC$.}
\begin{figure}[h!]
\begin{center}
	\begin{circuitikz}
...
\begin{align*}
	e(t) & = v(t) + RC\frac{\partial\,v(t)}{\partial\,t}\\
	E(p) & = V(p) + RC\,p\,V(p)\\
	V(p) & = \frac{1}{1+RC} E(p)
\end{align*}

On en déduit $H(p)$ puis $h(t)$.
...
\paragraph{Réponse du système à un échelon}

On pose $e(t) = E$, d'où $E(p) = \frac{E}{p}$.

\begin{align*}
	V(p) & = H(p)\times E(p)\\
	     & = \frac{1}{1+RC\,p}\times\frac{E}{p}
	     & = \frac{E}{RC}\left(\frac{a}{p}+\frac{b}{p+\frac{1}{RC}}\right)\qquad(\text{avec }a=RC\text{ et }b=-RC)\\
	     & = \frac{E}{p} - \frac{E}{p+\frac{1}{RC}}\\
\end{align*}

On peut alors retrouver $v(t)$ avec la transformée de Laplace inverse.

\begin{align*}
v(t) & = E\,\Gamma(t)-E\,e^{-\frac{T}{RC}}\\
     & = E\left(1-e^{-\frac{t}{RC}}\right)\Gamma(t)
\end{align*}

On peut de la même manière retrouver la réponse du circuit à tous types de signaux d'entrée.
\end{example}
\end{document}
